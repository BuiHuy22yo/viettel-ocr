<template>
  <div>
    <Space>
      <BasicTitle v-if="props.emptyHidePreview">
        {{ props.title || t('common.attachedFiles') }}
      </BasicTitle>
      <a-button
        type="primary"
        ...
        v-if="!props.isView"
      >
        {{ props.btnTitle || props.title || t('component.upload.upload') }}
      </a-button>
      <Tooltip placement="bottom" v-if="!props.emptyHidePreview">
        <template #title>
    ...
    <UploadPreview
      class="mt-4"
      :isView="props.isView"
      :isViewImg="props.isViewImg"
      v-if="props.emptyHidePreview"
      :value="fileList"
      :max-number="bindValue.maxNumber"
      @list-change="handlePreviewChange"
      @delete="handlePreviewDelete"
...
<script lang="ts" setup>
  ...
  function handleChange(files: any, valueKey: string) {
