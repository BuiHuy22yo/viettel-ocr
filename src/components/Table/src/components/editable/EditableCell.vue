<script lang="tsx">
  ...
  import { FormItemRest, Spin, Space } from 'ant-design-vue';
  import { parseRowKey } from '../../helper';

  export default defineComponent({
    name: 'EditableCell',
    components: { FormItemRest, FormOutlined, CloseOutlined, CheckOutlined, CellComponent, Spin },
    directives: {
      clickOutside,
    },
    props: {
    ...
    setup(props) {
      ...
      const getComponent = computed(() => {
        ...
        if (isFunction(editComponent)) {
          return editComponent({ record }) ?? {};
        }

        return editComponent || 'Input';
      });
