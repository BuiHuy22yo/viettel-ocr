...
<script lang="ts" setup>
  ...
  import FieldCustomerDrawer from './FieldCustomerDrawer.vue';

  import { columns } from './fieldCustomer.data';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';

  defineOptions({ name: 'FieldOfCustomer' });
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.fieldCustomer'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleFieldCustomer'),
    columns,
    bordered: true,
  ...
  function handleSuccess() {
    textSearch.value = '';
    searchAllGroupCustomer(textSearch.value);
  }
  ...
  async function searchAllGroupCustomer(text: string) {
    try {
      setLoading(true);
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      searchAllGroupCustomer(val.target.value.trim());
    }
  };
