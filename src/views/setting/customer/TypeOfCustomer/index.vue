...
<script lang="ts" setup>
  ...
  import CurrencyDrawer from './TypeCustomerDrawer.vue';

  import { columns } from './typeCustomer.data';
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.typeCustomer'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleTypeCustomer'),
    columns,
    bordered: true,
  ...
  function handleSuccess() {
    textSearch.value = '';
    searchAllTypeCustomer(textSearch.value);
  }
  ...
  async function searchAllTypeCustomer(text: string) {
    try {
      setLoading(true);
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      searchAllTypeCustomer(val.target.value.trim());
    }
  };
