    ...
    <BasicTable @register="registerTable">
      <template #toolbar>
        <div class="flex">
          ...
          <!--            ><Icon :icon="'ion:add-outline'" /> Add Type-->
          <!--          </a-button>-->
        </div>
      </template>
...
<script lang="ts" setup>
  ...
  import { deleteGlobalListDetail } from '@/api/sys/global_list';
  import { getDistrictByProvince } from '@/api/setting/location';
  ...
  import OrgDrawer from './TownshipDrawer.vue';

  import { columns } from './township.data';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';

  defineOptions({ name: 'Province' });
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.districtOrTownship'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleDistrict'),
  ...
  async function searchAllCompany(text: string) {
    try {
      setLoading(true);
      const data: any = await getDistrictByProvince({ keyword: text });
