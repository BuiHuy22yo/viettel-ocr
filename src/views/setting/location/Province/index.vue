...
<script lang="ts" setup>
  ...
  import { getAllProvince } from '@/api/setting/location';
  ...
  import OrgDrawer from './ProvinceDrawer.vue';

  import { columns } from './province.data';
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.regional'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleRegional'),
    icon: 'ph:city',
    columns,
    bordered: true,
  ...
  async function searchAllCompany(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllProvince({
        keyword: text,
      });
