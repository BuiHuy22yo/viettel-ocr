...
<script lang="ts" setup>
  ...
  import OrgDrawer from './NationDrawer.vue';

  import { columns } from './nation.data';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';
  import { getAllNation } from '@/api/setting/location';

  defineOptions({ name: 'Nation' });
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.nation'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleNation'),
  ...
  async function searchAllCompany(text: string) {
    try {
      setLoading(true);
      dataSource.value = await getAllNation({ keyword: text });
    } finally {
      setLoading(false);
    }
  }
