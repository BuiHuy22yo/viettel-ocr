      ...
      <template #toolbar>
        <div class="flex">
          ...
          <a-button type="primary" @click="handleCreate" class="ml-5"
            ...
            {{ t('serviceRequest.attribute_setting.modalAddTitle') }}
          </a-button>
        </div>
      </template>
...
<script lang="ts" setup>
  ...
  import { getCAllAttributes, deleteCDataSource } from '@/api/setting/service-request';
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('serviceRequest.attribute_setting.title'),
    dataSource: dataSource,
    subTitle: t('serviceRequest.attribute_setting.subTitle'),
    icon: 'carbon:group-objects-new',
    columns,
    bordered: true,
  ...
  async function searchAll(text: string) {
    try {
      setLoading(true);
      const data: any = await getCAllAttributes({
        code: text,
  ...
  const handleOk = async () => {
    try {
      ...
      await deleteCDataSource({ id: id.value });
