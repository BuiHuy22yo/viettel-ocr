      ...
      <template #toolbar>
        <div class="flex">
          ...
          <a-button type="primary" @click="handleCreate" class="ml-5"
            ><Icon :icon="'ion:add-outline'" /> {{ t('serviceRequest.type_setting.modalAddTitle') }}
          </a-button>
        </div>
      </template>
...
<script lang="ts" setup>
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('serviceRequest.type_setting.title'),
    dataSource: dataSource,
    subTitle: t('serviceRequest.type_setting.subTitle'),
  ...
  async function searchAll(text: string) {
    try {
      setLoading(true);
      const data: any = await searchRequestType({
        code: 'REQUEST_TYPE',
        name: text,
      });
  ...
  const handleOk = async () => {
    try {
      ...
      await deleteRequestType({ id: id.value });
