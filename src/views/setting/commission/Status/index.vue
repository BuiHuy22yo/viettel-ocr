...
<script lang="ts" setup>
  ...
  import { columns } from './status.data';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';

  defineOptions({ name: 'StatusCommission' });
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.statusCommission'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleStatusCommission'),
    columns,
    bordered: true,
  ...
  function handleSuccess() {
    textSearch.value = '';
    searchAllStatus(textSearch.value);
  }
  async function getParentId() {
    try {
      const data: any = await getOptGlobalListByCode({
        code: 'COMMISSION_STATUS',
      });
  ...
  async function searchAllStatus(text: string) {
    try {
      setLoading(true);
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      searchAllStatus(val.target.value.trim());
    }
  };
