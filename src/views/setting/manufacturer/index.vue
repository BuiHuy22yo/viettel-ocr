<template>
      ...
      <template #toolbar>
        ...
        <div class="flex">
          <a-button type="primary" class="ml-5" @click="handleAdd">
            <Icon :icon="'ion:add-outline'" /> {{ t('manufacturer.add') }}
          </a-button>
        </div>
      </template>
            ...
            :actions="[
              {
                ...
                tooltip: t('manufacturer.edit'),
                onClick: handleEdit.bind(null, record),
              },
              // {
              //   icon: 'ion:eye-outline',
              //   tooltip: t('manufacturer.view'),
              //   onClick: handleView.bind(null, record, false),
              // },
              {
                icon: 'ant-design:delete-outlined',
                tooltip: t('manufacturer.delete'),
                color: 'error',
  ...
  <AModal
    ...
    @ok="handleSubmit"
    :ok-text="t('manufacturer.add')"
    :cancel-text="t('manufacturer.cancel')"
  >
    <p class="p-5">
...
<script lang="ts" setup>
  ...
  import { columns, searchFormSchema, formEdit } from './manufacturer.data';
  ...
  async function handleAdd() {
    ...
    type.value = 'CREATE';
    titleAModal.value = t('manufacturer.create');
    await resetFields();
  }
  ...
  async function handleEdit(record: Recordable) {
    openModalEdit.value = true;
    type.value = 'UPDATE';
    titleAModal.value = t('manufacturer.update');
    await resetFields();
    record.isView = true;
  ...
  const handleOk = async () => {
    try {
      ...
      await deleteManufacturer({ id: id.value });
  ...
  async function downloadFile() {
    try {
      ...
      link.href = url;
      const fileName = `template_manufacturer.xlsx`;
