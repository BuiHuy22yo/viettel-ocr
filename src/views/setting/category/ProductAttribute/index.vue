...
<script lang="ts" setup>
  ...
  import productAttributeDrawer from './productAttributeDrawer.vue';

  import { columns } from './productAttribute.data';
  import { deleteProductAttribute, getAllProductAttribute } from '@/api/setting/service-product';
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.productAttribute'),
  ...
  async function searchAllProductAttribute(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllProductAttribute({
        keyword: text,
      });
  ...
  const handleOk = async () => {
    try {
      ...
      await deleteProductAttribute({ id: id.value });
      searchAllProductAttribute(textSearch.value);
