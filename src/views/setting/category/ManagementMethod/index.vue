...
<script lang="ts" setup>
  ...
  import DomainDrawer from './ManagementMethodDrawer.vue';

  import { columns } from './ManagementMethod.data';
  import { formatToDateTime } from '@/utils/dateUtil';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';
  import { deleteManagementMethod, getAllManagementMethod } from '@/api/setting/service-product';
  ...
  async function searchAllDomain(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllManagementMethod({
        keyword: text,
      });
  ...
  const handleOk = async () => {
    try {
      ...
      await deleteManagementMethod({ id: id.value });
      searchAllDomain(textSearch.value);
