...
<script lang="ts" setup>
  ...
  import DomainDrawer from './ShippingStatusDrawer.vue';

  import { columns } from './shippingStatus.data';
  import { formatToDateTime } from '@/utils/dateUtil';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';
  import { deleteShippingStatus, getAllShippingStatus } from '@/api/setting/service-product';
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.shippingStatus'),
  ...
  async function searchAllDomain(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllShippingStatus({
        keyword: text,
      });
  ...
  const handleOk = async () => {
    try {
      ...
      await deleteShippingStatus({ id: id.value });
      searchAllDomain(textSearch.value);
