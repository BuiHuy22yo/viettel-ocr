...
<script lang="ts" setup>
  ...
  import productAttributeDrawer from './groupAttributeDrawer.vue';

  import { columns } from './data';
  import { deletelGroupIndicator, getAllGroupIndicator } from '@/api/setting/service-product';
  import { formatToDateTime } from '@/utils/dateUtil';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';

  defineOptions({ name: 'GroupAttributeProduct' });
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.groupIndicator'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleGroupIndicator'),
  ...
  async function searchAllProductAttribute(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllGroupIndicator({
        keyword: text,
      });
  ...
  const handleOk = async () => {
    try {
      ...
      await deletelGroupIndicator({ id: id.value });
      searchAllProductAttribute(textSearch.value);
