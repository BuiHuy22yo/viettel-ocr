...
<script lang="ts" setup>
  ...
  import DomainDrawer from './UnitOfMeasureDrawer.vue';

  import { columns } from './unitOfMeasure.data';
  import { formatToDateTime } from '@/utils/dateUtil';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';
  import { deleteUnitOfMeasure, getAllUnitOfMeasure } from '@/api/setting/service-product';
  ...
  async function searchAllDomain(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllUnitOfMeasure({
        keyword: text,
      });
  ...
  const handleOk = async () => {
    try {
      ...
      await deleteUnitOfMeasure({ id: id.value });
      searchAllDomain(textSearch.value);
