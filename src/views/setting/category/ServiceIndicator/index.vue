<template>
  <div>
    ...
    <productAttributeDrawer @register="registerDrawer" @success="handleSuccess" />
    <AModal
...
<script lang="ts" setup>
  ...
  import productAttributeDrawer from './serviceIndicatorDrawer.vue';

  import { columns } from './data';
  import { deletelServiceIndicator, getAllServiceIndicator } from '@/api/setting/service-product';
  import { formatToDateTime } from '@/utils/dateUtil';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';

  defineOptions({ name: 'ServiceIndicator' });
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.serviceIndicator'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleServiceIndicator'),
  ...
  function handleDelete(record: Recordable) {
    ...
    id.value = record?.id;
    nameRecord.value = record?.indicatorName;
  }
  ...
  async function searchAllProductAttribute(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllServiceIndicator({
        keyword: text,
      });
  ...
  const handleOk = async () => {
    try {
      ...
      await deletelServiceIndicator({ id: id.value });
      searchAllProductAttribute(textSearch.value);
