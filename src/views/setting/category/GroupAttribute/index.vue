...
<script lang="ts" setup>
  ...
  import {
    deleteGroupProductAttribute,
    getAllGroupProductAttribute,
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.groupAttributeProduct'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleGroupAttributeProduct'),
  ...
  async function searchAllProductAttribute(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllGroupProductAttribute({
        keyword: text,
      });
  ...
  const handleOk = async () => {
    try {
      ...
      await deleteGroupProductAttribute({ id: id.value });
      searchAllProductAttribute(textSearch.value);
