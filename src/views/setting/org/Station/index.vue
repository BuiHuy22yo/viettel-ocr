...
<script lang="ts" setup>
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.station'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleStation'),
  ...
  function handleSuccess() {
    textSearch.value = '';
    searchStation(textSearch.value);
  }
  async function searchStation(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllStation({
        code: text,
        name: text,
      });
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      searchStation(val.target.value.trim());
    }
  };
  const handleOk = async () => {
    try {
      confirmLoading.value = true;
      await deleteStation({ id: id.value });
      searchStation(textSearch.value);
