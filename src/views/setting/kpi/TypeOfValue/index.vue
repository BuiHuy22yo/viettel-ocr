...
<script lang="ts" setup>
  ...
  import TypeOfValueDrawer from './TypeOfValueDrawer.vue';

  import { columns } from './typeOfValue.data';
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.typeOfValueKPI'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleTypeOfValueKPI'),
  ...
  function handleSuccess() {
    textSearch.value = '';
    searchAllTypeValue(textSearch.value);
  }
  ...
  async function searchAllTypeValue(text: string) {
    try {
      setLoading(true);
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      searchAllTypeValue(val.target.value.trim());
    }
  };
