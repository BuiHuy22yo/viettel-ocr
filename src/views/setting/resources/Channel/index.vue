...
<script lang="ts" setup>
  ...
  import RoleTitleDrawer from './ChannelDrawer.vue';

  import { columns } from './channel.data';
  import { deleteChannel, getAllChannel } from '@/api/setting/resources';
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.channel'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleChannel'),
    columns,
    bordered: true,
  ...
  function handleDelete(record: Recordable) {
    ...
    id.value = record?.id;
    nameRecord.value = record?.channelName;
  }

  function handleSuccess() {
    textSearch.value = '';
    searchAllChannel(textSearch.value);
  }
  async function searchAllChannel(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllChannel({
        keyword: text,
      });
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      searchAllChannel(val.target.value.trim());
    }
  };
  const handleOk = async () => {
    try {
      confirmLoading.value = true;
      await deleteChannel({ id: id.value });
      searchAllChannel(textSearch.value);
