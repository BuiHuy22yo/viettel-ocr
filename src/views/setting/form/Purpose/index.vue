      ...
      <template #toolbar>
        <div class="flex">
          ...
          <a-button type="primary" @click="handleCreate" class="ml-5"
            ><Icon :icon="'ion:add-outline'" /> {{ t('page.form.purpose.modalAddTitle') }}
          </a-button>
        </div>
      </template>
...
<script lang="ts" setup>
  ...
  import { getAllPurpose, deletePurpose } from '@/api/setting/template';
  ...
  import OrgDrawer from './PurposeDrawer.vue';

  import { columns } from './purpose.data';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';

  defineOptions({ name: 'Purpose' });
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('page.form.purpose.title'),
    dataSource: dataSource,
    subTitle: t('page.form.purpose.subTitle'),
  ...
  async function searchAll(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllPurpose({
        code: text,
      });
  ...
  const handleOk = async () => {
    try {
      ...
      await deletePurpose({ id: id.value });
