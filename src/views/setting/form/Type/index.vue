      ...
      <template #toolbar>
        <div class="flex">
          ...
          <a-button type="primary" @click="handleCreate" class="ml-5"
            ><Icon :icon="'ion:add-outline'" /> {{ t('page.form.type.modalAddTitle') }}
          </a-button>
        </div>
      </template>
...
<script lang="ts" setup>
  ...
  import { getAllType, deleteType } from '@/api/setting/template';
  ...
  import OrgDrawer from './TypeDrawer.vue';
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('page.form.type.title'),
    dataSource: dataSource,
    subTitle: t('page.form.type.subTitle'),
  ...
  async function searchAll(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllType({
        code: text,
      });
