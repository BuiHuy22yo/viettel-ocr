<template>
  <div>
    <BasicTable @register="registerTable" class="custom-table-body">
      <template #toolbar>
        <div class="flex">
          <a-input
          ...
          <a-button type="primary" @click="handleCreate" class="ml-5"
            ><Icon :icon="'ion:add-outline'" /> {{ t('page.form.groupAttributes.modalAddTitle') }}
          </a-button>
        </div>
      </template>
...
<script lang="ts" setup>
  ...
  import { getAllGroupAttributes, deleteGroupAttributes } from '@/api/setting/template';
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('page.form.groupAttributes.title'),
    dataSource: dataSource,
    subTitle: t('page.form.groupAttributes.subTitle'),
    icon: 'uit:object-ungroup',
    columns,
    bordered: true,
  ...
  async function searchAll(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllGroupAttributes({
        code: text,
      });
  ...
  const handleOk = async () => {
    try {
      ...
      await deleteGroupAttributes({ id: id.value });
