      ...
      <template #toolbar>
        <div class="flex">
          ...
          <a-button type="primary" @click="handleCreate" class="ml-5"
            ><Icon :icon="'ion:add-outline'" /> {{ t('page.form.extraData.modalAddTitle') }}
          </a-button>
        </div>
      </template>
...
<script lang="ts" setup>
  ...
  import { deleteTemplateExtraData, getAllTemplateExtraData } from '@/api/setting/template';
  ...
  import OrgDrawer from './ExtraDataDrawer.vue';

  import { columns } from './ExtraData.data';
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('page.form.extraData.title'),
    dataSource: dataSource,
    subTitle: t('page.form.extraData.subTitle'),
  ...
  async function searchAll(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllTemplateExtraData({
        code: text,
      });
  ...
  const handleOk = async () => {
    try {
      ...
      await deleteTemplateExtraData({ id: id.value });
