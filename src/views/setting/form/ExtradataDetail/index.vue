      ...
      <template #toolbar>
        <div class="flex">
          ...
          <a-button type="primary" @click="handleCreate" class="ml-5"
            ><Icon :icon="'ion:add-outline'" /> {{ t('page.form.extraDataDetail.modalAddTitle') }}
          </a-button>
        </div>
      </template>
...
<script lang="ts" setup>
  ...
  import {
    getAllTemplateExtraDataDetail,
    deleteTemplateExtraDataDetail,
  ...
  import OrgDrawer from './ExtraDataDetailDrawer.vue';

  import { columns } from './ExtraDataDetail.data';
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('page.form.extraDataDetail.title'),
    dataSource: dataSource,
    subTitle: t('page.form.extraDataDetail.subTitle'),
  ...
  async function searchAll(text: string) {
    try {
      setLoading(true);
      const data: any = await getAllTemplateExtraDataDetail({
        code: text,
      });
  ...
  const handleOk = async () => {
    try {
      ...
      await deleteTemplateExtraDataDetail({ id: id.value });
