      ...
      <template #toolbar>
        <div class="flex">
          ...
          <a-button type="primary" @click="handleCreate" class="ml-5">
            <Icon :icon="'ion:add-outline'" /> {{ t('paymentSetting.paymentDebtType.addButton') }}
          </a-button>
        </div>
      </template>
...
<script lang="ts" setup>
  ...
  import PeriodDrawer from './PeriodDrawer.vue';

  import { columns } from './data';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';

  defineOptions({ name: 'Payment Period' });
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('paymentSetting.paymentDebtType.title'),
    dataSource: dataSource,
    subTitle: t('paymentSetting.paymentDebtType.subTitle'),
    icon: 'lets-icons:group-scan',
    columns,
    bordered: true,
