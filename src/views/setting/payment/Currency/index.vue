...
<script lang="ts" setup>
  ...
  import CurrencyDrawer from './CurrencyDrawer.vue';

  import { columns } from './currency.data';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';

  defineOptions({ name: 'Currency' });
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.currency'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleCurrency'),
  ...
  function handleSuccess() {
    textSearch.value = '';
    searchAllCurrency(textSearch.value);
  }
  ...
  async function searchAllCurrency(text: string) {
    try {
      setLoading(true);
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      searchAllCurrency(val.target.value.trim());
    }
  };
