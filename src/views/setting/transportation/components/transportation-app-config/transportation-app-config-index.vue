    ...
    <BasicTable @register="registerTable" class="w-4/4 xl:w-5/5 custom-table-body">
      ...
      <template #bodyCell="{ column, record }">
            ...
            :actions="[
              {
                ...
                tooltip: t('transportation.transportationAppConfig.table.action.view'),
                onClick: handleView.bind(null, record),
              },
              {
                icon: 'clarity:note-edit-line',
                tooltip: t('transportation.transportationAppConfig.table.action.edit'),
                onClick: handleEdit.bind(null, record),
              },
              {
                icon: 'ant-design:delete-outlined',
                tooltip: t('transportation.transportationAppConfig.table.action.delete'),
                color: 'error',
        ...
        <template v-if="column.key === 'stockType'">
          {{
            record.stockType === 0
              ? t('transportation.transportationAppConfig.common.stockTypeList.stockParent')
              : t('transportation.transportationAppConfig.common.stockTypeList.affiliatedStock')
          }}
        </template>
      </template>
    </BasicTable>
...
<script lang="ts" setup>
  ...
  import { columns } from './transportation-app-config.data';
  ...
  import transportationAppConfigDrawer from './transportation-app-config-drawer/transportation-app-config-drawer.vue';
  import HeaderCell from '@/components/Table/src/components/HeaderCell.vue';
  import {
    transportationAppConfigDeleteApi,
    transportationAppConfigSearchApi,
  ...
  const [registerTable, { setLoading, getForm, getPaginationRef, setPagination }] = useTable({
    title: t('transportation.transportationAppConfig.table.title'),
    subTitle: t('transportation.transportationAppConfig.table.subTitle'),
    ...
    actionColumn: {
      width: 100,
      title: t('transportation.transportationAppConfig.table.columns.action'),
      dataIndex: 'action',
      align: 'center',
    },
  });
  ...
  const handleOk = async () => {
    try {
      ...
      await transportationAppConfigDeleteApi({
  ...
  const handleSearch = async (params: any) => {
    try {
      setLoading(true);
      dataSource.value = await transportationAppConfigSearchApi({
        keyword: params,
      });
    } finally {
      setLoading(false);
