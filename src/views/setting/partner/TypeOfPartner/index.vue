...
<script lang="ts" setup>
  ...
  import TypeOfPartnerDrawer from './TypeOfPartnerDrawer.vue';

  import { columns } from './typeOfPartner.data';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';

  defineOptions({ name: 'TypeOfPartner' });
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.typeOfPartner'),
    dataSource: dataSource,
    subTitle: t('form.setting.subTitleTypeOfPartner'),
  ...
  function handleSuccess() {
    textSearch.value = '';
    searchAllTypePartner(textSearch.value);
  }
  ...
  async function searchAllTypePartner(text: string) {
    try {
      setLoading(true);
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      searchAllTypePartner(val.target.value.trim());
    }
  };
