...
<script lang="ts" setup>
  ...
  import RatingIndicatorDrawer from './AttributeDrawer.vue';

  import { columns } from './data';
  import { getAllAttribute, deleteAttribute } from '@/api/attribute';
  ...
  const [registerTable, { setLoading }] = useTable({
    ...
    subTitle: t('form.setting.subTitleAttribute'),
  ...
  function handleSuccess() {
    textSearch.value = '';
    searchAllAttribute(textSearch.value);
  }
  async function searchAllAttribute(text: string) {
    try {
      setLoading(true);
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      searchAllAttribute(val.target.value.trim());
    }
  };
