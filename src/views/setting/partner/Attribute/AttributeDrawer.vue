...
<script lang="ts" setup>
  ...
  const getTitle = computed(() =>
    !unref(isUpdate) ? t('form.setting.createAttribute') : t('form.setting.updateAttribute'),
  );
  ...
  async function handleSubmit() {
    try {
      ...
      !unref(isUpdate)
        ? await createAttribute({ ...values, type: 3 })
        : await updateAttribute({ ...dataRecord.value, ...values, type: 3 });
      closeDrawer();
