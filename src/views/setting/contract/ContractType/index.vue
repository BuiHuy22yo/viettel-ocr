...
<script lang="ts" setup>
  ...
  import StatusDrawer from './ContractTypeDrawer.vue';

  import { columns } from './contractType.data';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';

  defineOptions({ name: 'ContractStatus' });
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.contractType'),
    dataSource: dataSource,
    // subTitle: t('form.setting.subTitleContractStatus'),
  ...
  function handleSuccess() {
    textSearch.value = '';
    searchAllContractStatus(textSearch.value);
  }
  ...
  async function searchAllContractStatus(text: string) {
    try {
      setLoading(true);
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      searchAllContractStatus(val.target.value.trim());
    }
  };
