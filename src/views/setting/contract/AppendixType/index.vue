...
<script lang="ts" setup>
  ...
  import StatusDrawer from './AppendixTypeDrawer.vue';

  import { columns } from './appendixType.data';
  ...
  const [registerTable, { setLoading }] = useTable({
    title: t('form.setting.appendixType'),
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      await searchAll(val.target.value.trim());
    }
  };
  const handleOk = async () => {
    try {
      ...
      await searchAll(textSearch.value);
