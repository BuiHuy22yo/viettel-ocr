...
<script lang="ts" setup>
  ...
  import TypeOfDebitDrawer from './TypeOfDebtDrawer.vue';

  import { columns } from './typeOfDebt.data';
  ...
  function handleSuccess() {
    textSearch.value = '';
    searchAllTypeOfDebt(textSearch.value);
  }
  ...
  async function searchAllTypeOfDebt(text: string) {
    try {
      setLoading(true);
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      await searchAllTypeOfDebt(val.target.value.trim());
    }
  };
  ...
  const handleOk = async () => {
    try {
      ...
      await searchAllTypeOfDebt(textSearch.value);
