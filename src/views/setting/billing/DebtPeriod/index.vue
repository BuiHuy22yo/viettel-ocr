...
<script setup lang="ts">
  ...
  import DebtPeriodDrawer from './DebtPeriodDrawer.vue';

  import { columns } from './debtPeriod.data';
  import { message } from 'ant-design-vue';
  import { useI18n } from '@/hooks/web/useI18n';

  defineOptions({ name: 'DebtPeriod' });
  ...
  const [registerTable, { setLoading }] = useTable({
    ...
    subTitle: 'Setup your billing for your business',
    columns,
    bordered: true,
  ...
  function handleSuccess() {
    textSearch.value = '';
    searchAllDebtPeriod(textSearch.value);
  }
  ...
  async function searchAllDebtPeriod(text: string) {
    try {
      setLoading(true);
  ...
  const onChangeTextSearch = async (val) => {
    ...
    if (value === 'response') {
      searchAllDebtPeriod(val.target.value.trim());
    }
  };
